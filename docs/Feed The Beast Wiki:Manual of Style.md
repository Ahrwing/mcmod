...
这种垃圾是被严格禁止的。任何提交了含有此类内容的编辑者都会被ban掉。
...
对于其他维基的敌意行为是不可容忍的。对其他维基发出敌意评论的编辑者会被警告，严重者会被ban掉，即便是维基的员工犯下此类错误也会被除名并被ban。
...
不禁止指向其他维基的链接，如果你认为这样的链接对于提升编辑的质量有帮助的话。相关友链可以在“项目：管理员公告板”上找到。
...
但是建议编辑者一次只引用一条维基链接，以免无意间触犯所引用的维基自己的版权政策。
...
Capitalize letters in a title as they would be capitalized in a sentence.x

The first letter of a title will always be automatically capitalized.x
...
Capitalize letters in headings as they would be capitalized in a sentence.x
...
如果你想编辑wiki但英语不是你的本土语言，而你又不自信能使用好它，考虑翻译维基吧。
...
Use American English, such as armor rather than armour. Use British English by translating it.x
...
永远不要在文章里去更正引用的游戏内文本里的各种格式错误。游戏内文本或者物品名应当一直保持与游戏里的严格一致。这一点已经在本手册里多次提到。不过与其更正语法错误，还是使用{{Sic}}标记出来，并考虑通知内容的原作者有关格式错误的问题。

Use modpack instead of mod pack or mod-pack.x

Use multiblock instead of multi-block or multi block.x
...
目录链接应当置于页面尾部，但是在要求的位置之前，加入<languages/>标记以隔开目录链接和其他部分。
...
请不要擅自创建新目录。如果你没法在已有目录里找到合适的，请在“员工公告板”里提出添加新目录的请求。多余的目录将会在被删除而不会有另行通知。
...
There are no naming conventions for images that exist on pages, however there are certain conventions for interface images.
 对于已经存在于页面上的图片没有什么命名规定，但是对于界面图片还是有特定的规定的。
...
维基标记通常称为wikitext。一篇文章的源码应当是清晰且易于阅读的。
